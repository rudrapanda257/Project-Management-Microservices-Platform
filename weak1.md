## **DAY 2: User Service - JWT Authentication**

### **🎯 Goals:**

- Complete User Service with registration and login
...
### **🤖 AI Prompts for Day 2:**

**Prompt 1: User Service POM**

```

Create a pom.xml for user-service module with:

- Parent: task-management-platform version 1.0.0
...
Create a User JPA entity class for Spring Boot with:

- Annotations: @Entity, @Table(name = "users"), @Data, @NoArgsConstructor, @AllArgsConstructor
- Fields:
  - id (Long, @Id, @GeneratedValue IDENTITY)
  - email (String, @Column unique, not null)
  - password (String, not null)
  - name (String, not null)
  - role (Enum: ADMIN, MANAGER, MEMBER) with @Enumerated(STRING)
  - createdAt (LocalDateTime, @Column with columnDefinition)
- Add @PrePersist method to set createdAt
- Package: com.taskmanager.user.entity

```

**Prompt 3: JWT Utility Class**

```

Create a JwtUtil class for Spring Boot with these methods:

1. generateToken(String email, String role) - creates JWT with 24hr expiration
2. extractEmail(String token) - extracts email from token
3. extractRole(String token) - extracts role from token
4. validateToken(String token, String email) - validates token
5. isTokenExpired(String token) - checks expiration

Use:

- Secret key from application properties
- HS256 algorithm
- Include email and role in claims
- Package: com.taskmanager.user.security
- Use io.jsonwebtoken library

```

**Prompt 4: Auth Controller**

```

Create an AuthController REST controller with:
...
1. POST /register - accepts RegisterRequest (email, password, name, role)
   Returns: AuthResponse (token, email, role, name)
2. POST /login - accepts LoginRequest (email, password)
   Returns: AuthResponse (token, email, role, name)
...
- Methods:
  1. register(RegisterRequest) - validates email unique, encodes password, saves user, generates token
  2. login(LoginRequest) - validates credentials, generates token
...
Create a SecurityConfig class for Spring Security with:

- @Configuration, @EnableWebSecurity
- Define SecurityFilterChain bean
- Permit: /api/auth/\*\* (public)
...
**Prompt 7: JWT Authentication Filter**
...
**Prompt 8: Application Configuration**

```

Create application.yml for user-service with:
...
- JPA:
  ...
  - show-sql: true
  - properties: dialect PostgreSQL
- JWT secret: use environment variable with default
- Logging level: INFO for app, DEBUG for SQL
...
Create these DTO classes in com.taskmanager.user.dto package:
...
**Prompt 10: Global Exception Handler**

```

Create GlobalExceptionHandler with @RestControllerAdvice:

- Handle UserAlreadyExistsException - return 409 CONFLICT
- Handle InvalidCredentialsException - return 401 UNAUTHORIZED
- Handle MethodArgumentNotValidException - return 400 BAD_REQUEST with field errors
- Handle generic Exception - return 500 INTERNAL_SERVER_ERROR
- Package: com.taskmanager.user.exception
- Return consistent error response format: {timestamp, status, message, details}

```

**Prompt 11: Main Application Class**

```

Create UserServiceApplication class:

- @SpringBootApplication annotation
- main method with SpringApplication.run
- Package: com.taskmanager.user
  💻 Manual Tasks for Day 2:

1. Create all folders in user-service/src/main/java/com/taskmanager/user/
2. Copy AI-generated code into respective files
...
- [ ] Postman collection created with tests
